...
test.describe("Form Validation Error Display", () => {
  ...
  test.describe("Category Form Validation", () => {
    ...
    test("shows error when category name is only whitespace", async ({ page }) => {
      await page.getByRole("button", { name: "Add Category" }).click()
  ...
  test.describe("Transaction Form Validation", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.goto("/accounts")
      const hasAccounts = await page.getByRole("table").isVisible().catch(() => false)
    ...
    test("shows error when transaction type is not selected", async ({ page }) => {
      await page.getByRole("button", { name: "Add Transaction" }).click()
...
test.describe("API Error Handling and User Feedback", () => {
  test.describe("Success Messages", () => {
    ...
    test("shows success toast when account is deleted", async ({ page }) => {
      ...
      // Should show success toast
      await expect(page.getByText("The account was deleted successfully")).toBeVisible()
    })
  })
  ...
  test.describe("Category Error Messages", () => {
    ...
    test("shows success toast when category is deleted", async ({ page }) => {
      ...
      // Should show success toast
      await expect(page.getByText("The category was deleted successfully")).toBeVisible()
    })
  })
  ...
  test.describe("Transaction Error Messages", () => {
    ...
    test("shows success toast when transaction is created", async ({ page }) => {
    ...
    test("shows success toast when transaction is deleted", async ({ page }) => {
      ...
      const transactionRow = page.getByRole("row").filter({ hasText: description })
      await transactionRow.getByRole("button").last().click()
      await page.getByRole("menuitem", { name: "Delete Transaction" }).click()
      await page.getByRole("button", { name: "Delete" }).click()
      
      // Should show success toast
      await expect(page.getByText("The transaction was deleted successfully")).toBeVisible()
    })
  })
})
